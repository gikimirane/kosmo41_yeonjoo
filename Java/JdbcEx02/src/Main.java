...
public class Main {
	...
	public static void main(String[] args) {
		...
		try {
			...
			sb.append("create table test1 ( "); // 뒤에 공백이 있어야, 한문장으로 인식하지 않고 따로따로 인식함
			...
			sb.setLength(0);
			sb.append("select * from test1");
			rs = stmt.executeQuery(sb.toString());  // 업데이트를 여러개 받을 때 사용 
			while (rs.next()) {
				System.out.print("id : " + rs.getString(1) + ", ");
				System.out.println("age : " + rs.getString("age"));
			}
			...
			updateCount = stmt.executeUpdate(sb.toString());   // 업데이트를 하나만 받을 때 사용
